...
const PageHero = ({ title, subtitle, className = "" }: { title: string; subtitle?: string; className?: string }) => {
    return (
            ...
            <Section className="text-center mb-16 animate-on-scroll opacity-0 w-full max-w-[1280px] mx-auto">
                <div className="max-w-4xl mx-auto">
                    ...
                    <p className="text-xl text-gray-300 font-light leading-relaxed">
                        At TED<sup>x</sup>UOK, we believe that ideas have the power to change attitudes,
                        lives, and ultimately, the world. Our social impact initiatives aim
                        to foster sustainability, community growth, and inclusive dialogue,
                        creating a ripple effect of positive change that extends far beyond
                        our event.
                    </p>
                </div>
            </Section>
                    ...
                    {[
                        {
                            ...
                            description: "A student-led movement to reduce single-use plastics and implement better waste segregation systems."
                        },
                        {
                            id: 2,
                            title: "Tech for Good",
                            img: "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&q=80&w=600",
                            description: "Workshops and hackathons focused on developing open-source solutions for local non-profits."
                        },
                        {
                            id: 3,
                            title: "Literacy Outreach",
                            img: "https://images.unsplash.com/photo-1497633762265-9d179a990aa6?auto=format&fit=crop&q=80&w=600",
                            description: "Weekend mentorship programs where university volunteers help underprivileged school children."
                        }
                    ].map((project, index) => (
                        <div key={project.id} style={{ animationDelay: `${index * 150}ms` }} className="animate-on-scroll opacity-0 h-full">
                    ...
                    {[
                        ...
                        <div key={index} className="p-6">
                            <div className="text-5xl font-bold text-[#EB0028] mb-2">
                                <CountUp end={parseInt(stat.value.replace(/\D/g, ''))} />{stat.value.replace(/[0-9]/g, '')}
                            </div>
                            <div className="text-gray-300 font-light uppercase tracking-widest text-sm">{stat.label}</div>
                        </div>
                    ))}
                </div>
            </Section>
            ...
            <Section className="mb-16 animate-on-scroll opacity-0 w-full max-w-[1280px] mx-auto">
                <div className="bg-[#1a1a1a] border border-[#333] rounded-2xl p-12 text-center">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <a href="/registration" className="px-8 py-3 bg-[#EB0028] text-white font-bold rounded-full hover:bg-white hover:text-[#EB0028] transition duration-300 text-center">
                            Become a Volunteer
                        </a>
                        <a href="/partners" className="px-8 py-3 border border-white text-white font-bold rounded-full hover:bg-white hover:text-black transition duration-300 text-center">
                            Partner With Us
                        </a>
                    </div>
                </div>
